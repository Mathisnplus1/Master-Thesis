...
def get_FMNIST_loaders(path, class_names, batch_size) :
    ...
    # load MNIST 
    mnist_train = datasets.FashionMNIST(root=path, train=True, download=False, transform=transform)
    mnist_test = datasets.FashionMNIST(root=path, train=False, download=False, transform=transform)
