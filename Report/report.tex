...
\begin{itemize}
    ...
    \item so, in a nutshell : we assume we keep absolutely no memory of any kind of the previous tasks except for the weights of the old neurons
\end{itemize}

So our very last hope is to parametrize/constrain the weights so that they encode task information.

\subsection{Conclusions we can draw from these attempts}

Approaches like LoRA, EWC, neural growth (whether initialization is done through GradMax of TINY) are condemned to fail in the context of continual learning. While they have no information about previous tasks samples, they will be limited to trade offs and hyperparameter optimization.
