...
\noindent
which allows to define the pollution of samples from task $i$ through protected weights for task $j$ on neuron $n$ of layer $l$ as
\begin{align}
    p_{l,n}^{i\rightarrow j} = \mathbb{E}_{x\in X_i}\left[\frac{\sum_{w \in \mathbb{W}_{l,n}^j}{w(x)}}{\sum_{w \in \mathbb{W}_{l,n}^i}{w(x)}}\right]
\end{align}

\noindent
Finally, we can define the pollution of samples from task $i$ through protected weights for task $j$ on the whole network as
\begin{align}
